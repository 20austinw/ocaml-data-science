...
(** [construct m] takes in a float list list m and returns matrix. Throws
 InvalidDimensions if rows are not of the same length *)

val lu_decomp : t -> t * t
(** [decomp m] produces a pair of matrices containing the LU-decomposition 
of matrix m*)
...
val invert : t -> t
(** [invert m] returns the inverse of matrix m. Throws InvalidDimensions if 
...
(** [eigen mat dom] returns a pair float * t containing the eigenvector and its 
associated eigenvalue. If dom is true then function returns the dominant 
eigenvector, else the smallest. *)

val elem_pow : t -> float -> t
(** [elem_pow m r] raises each entry of m to the power of r and returns the 
resulting matrix.*)

val scale : t -> float -> t
(** [scale m c] takes in m and scales it by factor of
    c*)
