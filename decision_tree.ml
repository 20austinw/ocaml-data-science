...
let find_index_in_lst elem lst = 
  ...
  in find_index elem lst 0

let leaf_value_calculate y = 
  let classes = List.sort_uniq compare y in 
  let count_lst = 
    List.map (fun x -> 
      float_of_int (num_occurence_in_lst x y)) classes in 
...
let impurity_calculate y y1 y2 = 
  let p = float_of_int (List.length y1) 
    /. float_of_int (List.length y2) in
...
let to_single_col y = 
  ...
  | [ x ] -> x 
  | _ -> failwith "Cannot convert to single column") y

let find_best_criteria_sets criteria_sets_lst = 
  let impurities_lst = 
    List.map (fun (impurity, criteria, set) -> 
      impurity) criteria_sets_lst in
  let largest_impurity = max_of_lst impurities_lst in
  let largest_index = find_index_in_lst largest_impurity impurities_lst in
  List.nth criteria_sets_lst largest_index

let rec build_tree x y current_depth = 
  let y_as_float_lst_lst = List.map (fun x -> [ x ]) y in
  let xy = y_as_float_lst_lst 
  |> construct 
  |> concat (construct x) in 
  let cols = xy 
  |> transpose 
  |> matrix in
  ...
  if n_samples < min_samples_split && current_depth > max_depth then
    ...
    let best_values_for_features = List.mapi (fun i col -> 
      let unique_values = List.sort_uniq compare col in 
      let criteria_sets_lst = List.map (fun t -> 
      ...
      let best_values_for_threshold = 
        find_best_criteria_sets criteria_sets_lst in
      best_values_for_threshold  
    ) cols in
    let largest_impurity, best_c, best_s =
        find_best_criteria_sets best_values_for_features in
    if largest_impurity > min_impurity then 
      let t_branch = 
        build_tree best_s.left_x best_s.left_y (current_depth + 1) in
      let f_branch = 
        build_tree best_s.right_x best_s.right_y (current_depth + 1) in 
      {
  ...
  | None ->
    begin 
      let f_i = 
        ...
        | Some x -> x
        | None -> failwith "This value can never be none" in
      let th = 
      ...
      let branch = 
        if feature_value >= th then t_branch else f_branch in 
      predict_value row branch
    end 
  
let predict x y = 
  let tree = fit x y in 
  List.map (fun row -> predict_value row tree) x